...
// LOAD BATTLE THREE FUNCTION
function loadBattleThree() {
  // create div for menu container
  battleContainerEl = document.createElement("div");
  battleContainerEl.className = "content-container";
  battleContainerEl.setAttribute("id", "battle-container");
  ...
  knightImageContainerEl.className = "character-image-container";
  knightImageContainerEl.setAttribute("id", "knight-image-container");
  // add knight image for battle three
  var knightImageThree = document.createElement("img");
  knightImageThree.src = "./assets/images/characters/fight3.png";
  knightImageThree.setAttribute("id", "knight-three");
  knightImageThree.className = "character-img";
  knightImageContainerEl.appendChild(knightImageThree);
  // append knight image container to battle container
  battleImageContainerEl.appendChild(knightImageContainerEl);
  // create div for goblin image
  var goblinImageContainerEl = document.createElement("div");
  goblinImageContainerEl.className = "character-image-container";
  goblinImageContainerEl.setAttribute("id", "goblin-image-container");
  // add goblin image for battle three
  var goblinFightImage = document.createElement("img");
  goblinFightImage.src = "./assets/images/characters/goblinfight.png";
  goblinFightImage.setAttribute("id", "goblin-fight");
  goblinFightImage.className = "character-img";
  goblinImageContainerEl.appendChild(goblinFightImage);
  // append goblin image container to battle container
  battleImageContainerEl.appendChild(goblinImageContainerEl);
  // append image container to battle container
  battleContainerEl.appendChild(battleImageContainerEl);
  ...
  var fightBtn = document.createElement("button");
  fightBtn.className = "fight-btn";
  fightBtn.setAttribute("id", "battleThree-fight-btn");
  fightBtn.textContent = "Fight!";
  btnContainerEl.appendChild(fightBtn);
  battleContainerEl.appendChild(btnContainerEl);
  ...
  var battleTextDivEl = document.createElement("div");
  battleTextDivEl.className = "text-box";
  battleTextDivEl.innerHTML = "<b>BATTLE THREE</b>: Goblin Warrior Steve";
  battleTextDivEl.setAttribute("id", "battle-text-box");
  battleContainerEl.appendChild(battleTextDivEl);
  ...
  // BATTLE THREE EVENT LISTENER
  fightBtn.addEventListener("click", function () {
    // play random fight noise
    function randomSound() {
      var random = Math.floor(Math.random() * goblinBattleSounds.length);
      goblinBattleSounds[random].play();
    }
    randomSound();
    // check player health
    if (playerInfo.health > 0) {
      ...
      if (playerInfo.defense > 0) {
        // subtract enemy attack from defense
        playerInfo.defense = goblinInfo.attack - playerInfo.defense;
        // attack goblin
        goblinInfo.health = goblinInfo.health - playerInfo.attack;
        // check to see if defense is at zero
        if (playerInfo.defense <= 0) {
          // take remaining attack and subtract from health
          playerInfo.health = playerInfo.defense + playerInfo.health;
          ...
          newDefAttackText.className = "info-text";
          newDefAttackText.innerHTML =
            "<ion-icon name='arrow-forward-outline'></ion-icon> <b>Sir. Prise</b> attacked <b>Steve</b> by " +
            playerInfo.attack +
            " points! <b>Steve</b> attacked <b>Sir. Prise</b> by " +
            goblinInfo.attack +
            " points! <b>Steve</b> has " +
            goblinInfo.health +
            " health left! Remaining defense added to health!";
          battleTextDivEl.appendChild(newDefAttackText);
        } else {
        ...
        // regular attack
        playerInfo.health = playerInfo.health - goblinInfo.attack;
        // attack goblin
        goblinInfo.health = goblinInfo.health - playerInfo.attack;
        // update stats bar
        healthStatsDivEl.textContent = playerInfo.health;
        ...
        battleTextDivEl.appendChild(newAttackText);
        // check goblin health
        if (goblinInfo.health <= 0) {
          // win battle scene
          // change images
          knightImageThree.src = "./assets/images/characters/win.png";
          goblinFightImage.src = "./assets/images/characters/goblinko.png";
          // play sound
          winBattleSound.play();
          // add money
          playerInfo.money = playerInfo.money + 20;
          ...
          newWinText.innerHTML =
            "<ion-icon name='arrow-forward-outline'></ion-icon> <b>Sir. Prise</b> has won the battle! 20 coins has been added to your inventory!";
          battleTextDivEl.appendChild(newWinText);
          // change button to go win page
          fightBtn.remove();
          var winGameBtn = document.createElement("button");
          winGameBtn.className = "green-btn";
          winGameBtn.setAttribute("id", "battleThree-menu-btn");
          winGameBtn.textContent = "You Win!";
          btnContainerEl.appendChild(winGameBtn);
          // event listener to go to win game page
          winGameBtn.addEventListener("click", function () {
            battleContainerEl.remove();
            battleMusic.pause();
            loadWin();
        ...
        if (playerInfo.health <= 0) {
          // lose battle scene
          // change images
          knightImageThree.src = "./assets/images/characters/ko.png";
          goblinFightImage.src = "./assets/images/characters/goblinwin.png";
          // play sound
          loseGameSound.play();
          // print lose text to text box
          var newLoseText = document.createElement("p");
      ...
      var endGameBtn = document.createElement("button");
      endGameBtn.className = "fight-btn";
      endGameBtn.setAttribute("id", "battleThree-end-btn");
      endGameBtn.textContent = "Try Again";
      btnContainerEl.appendChild(endGameBtn);
      // event listener to end game
      endGameBtn.addEventListener("click", function () {
